...
class Learning extends Component {
    ...
    componentDidMount() {
        ...
        var query = `
        query {
            productTypes(where:{product_type:"Bundle"}){
              id,
              product_type,
              product_topics{
                id,
        ...
        this.setState({
            productsGet: {
                ...
                data: (relatedLearning && relatedLearning[0]) ? relatedLearning[0].product_topics[index].product.filter((item) => {
                    if (item.product_type != null) {
                        return (item.product_type.product_type === 'Bundle') && (item.product_slug !== splitData[1] + '-' + splitData[2]) && (item.product_topic.product_topic_name === product_topics)
                    }
                }) : [],
            }
        })
