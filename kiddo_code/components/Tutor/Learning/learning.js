...
class Learning extends Component {
    ...
    componentDidMount() {
        ...
        var query = `
        query {
            productTypes(where:{product_type:"Tutor"}){
              id,
              product_type,
              product_topics{
                id,
            ...
            .then((response) => {
                ...
                let products = getProductDataFilter[0].product_topics[0].product.filter((item) => {
                    if (item.product_type != null) {
                        return (item.product_type.product_type === 'Tutor') && (item.product_slug !== splitData[1] + '-' + splitData[2]) && (item.product_topic.product_topic_name === 'Coding')
                    }
                })
        ...
        this.setState({
            productsGet: {
                ...
                data: (relatedLearning && relatedLearning[0]) ? relatedLearning[0].product_topics[index].product.filter((item) => {
                    if (item.product_type != null) {
                        return (item.product_type.product_type === 'Tutor') && (item.product_slug !== splitData[1] + '-' + splitData[2])
                    }
                }) : [],
            }
        })
                    ...
                    <div className="container">
                        <div className="row justify-content-center text-center mb-3">
                            <div className="col-xl-8 col-lg-9">
                                <h2 className="mx-xl-8">Related items</h2>
                            </div>
                        </div>
